...
```text
Release ONDEWO S2T Nodejs Client ${ONDEWO_S2T_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO S2T Nodejs Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO S2T Nodejs Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/s2t-client-nodejs`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/s2t-client-nodejs versions --json
```
